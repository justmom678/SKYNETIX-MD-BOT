#!/bin/bash
# SKYNETIX -MD Heroku One-Click Deployer
# Usage: bash <(curl -s https://raw.githubusercontent.com/justmom678/SKYNETIX -MD/main/lib/heroku.sh)
...
echo -e "${CYAN}"
echo "╔═══════════════════════════════════════╗"
echo "║       SKYNETIX -MD Heroku Deployer         ║"
echo "║         by justmom678             ║"
echo "╚═══════════════════════════════════════╝"
echo -e "${NC}"
...
# App name
read -p "$(echo -e ${CYAN}App name (e.g. my-SKYNETIX -md): ${NC})" APP_NAME
APP_NAME=${APP_NAME:-SKYNETIX -md-bot}

# Session ID
read -p "$(echo -e ${CYAN}Session ID (justmom678/SKYNETIX -MD_xxxxx): ${NC})" SESSION_ID
...
# Owner number
read -p "$(echo -e ${CYAN}Owner WhatsApp number (e.g. 234001234567): ${NC})" OWNER_NUMBER
OWNER_NUMBER=${OWNER_NUMBER:-234000000000}

# Bot name
read -p "$(echo -e ${CYAN}Bot name (default: SKYNETIX -MD): ${NC})" BOT_NAME
BOT_NAME=${BOT_NAME:-SKYNETIX -MD}

# MongoDB
read -p "$(echo -e ${CYAN}MongoDB URL (recommended, press Enter to skip): ${NC})" MONGO_URL

# Timezone
read -p "$(echo -e ${CYAN}Timezone (default: Africa/Harare): ${NC})" TIMEZONE
...
if [ ! -f "heroku.yml" ]; then
    echo -e "${YELLOW}📦 Cloning SKYNETIX -MD repo...${NC}"
    git clone https://github.com/justmom678/SKYNETIX -MD SKYNETIX -md-deploy
    cd SKYNETIX -md-deploy
fi
