#!/bin/bash
# SKYNETIX-MD Railway One-Click Deployer
# Usage: bash <(curl -s https://raw.githubusercontent.com/SKYNETIX/SKYNETIX/main/lib/railway.sh)
...
echo -e "${CYAN}"
echo "╔═══════════════════════════════════════╗"
echo "║       SKYNETIX-MD Railway Deployer        ║"
echo "║          by kudzie            ║"
echo "╚═══════════════════════════════════════╝"
echo -e "${NC}"
...
read -p "$(echo -e ${CYAN}Session ID (sky/skynetix_xxxxx): ${NC})" SESSION_ID
...
OWNER_NUMBER=${OWNER_NUMBER:-263000000000}
...
railway init --name "Skynetix-md-bot"
