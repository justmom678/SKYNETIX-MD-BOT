...
export default {
    ...
    aliases: ['Skynetixdl'],
    category: 'download',
    description: 'Download from Skynetix with real-time progress',
    usage: '.Skynetix <Skynetix-url>',
    async handler(sock, message, args, context) {
        const chatId = context.chatId || message.key.remoteJid;
        const text = args.join(' ').trim();
        if (!text) {
            return sock.sendMessage(chatId, { text: `*Usage:* .Skynetix https://Skynetix.nz/file/xxxx#xxxx` }, { quoted: message });
        }
        try {
