...
vi.mock('../../config.js', () => ({ default: { ownerNumber: '263776695904@s.whatsapp.net', prefixes: ['.'], botName: 'SKYNETIX Bot', botMode: 'public' } }));
...
describe('handleMessages', () => {
    ...
    it('sends error response when command handler throws', async () => {
        ...
        const msg = createMockMessage({ text: '.crashtest', isGroup: true, senderId: '263776695904@s.whatsapp.net' });
        await handleMessages(sock, { messages: [msg], type: 'notify' });
...
describe('handleGroupParticipantUpdate', () => {
    ...
    it('handles add without crashing', async () => {
        await expect(handleGroupParticipantUpdate(sock, {
            id: '120363422153603317@g.us',
            participants: ['263776695904@s.whatsapp.net'],
            action: 'add', author: '263776695904@s.whatsapp.net',
    ...
    it('handles remove without crashing', async () => {
        await expect(handleGroupParticipantUpdate(sock, {
            id: '120363422153603317@g.us',
            participants: ['263776695904@s.whatsapp.net'],
            action: 'remove', author: '263776695904@s.whatsapp.net',
        })).resolves.not.toThrow();
    });
    ...
    it('handles promote without crashing', async () => {
        await expect(handleGroupParticipantUpdate(sock, {
            id: '120363422153603317@g.us',
            participants: ['263776695904@s.whatsapp.net'],
            action: 'promote', author: '263776695904@s.whatsapp.net',
        })).resolves.not.toThrow();
    });
    ...
    it('handles demote without crashing', async () => {
        await expect(handleGroupParticipantUpdate(sock, {
            id: '120363422153603317@g.us',
            participants: ['263776695904@s.whatsapp.net'],
            action: 'demote', author: '263776695904@s.whatsapp.net',
        })).resolves.not.toThrow();
    });
    ...
    it('invalidates group cache on any update', async () => {
        ...
        await handleGroupParticipantUpdate(sock, {
            id: '120363422153603317@g.us', participants: [], action: 'add', author: '',
        });
        expect(invalidateGroupCache).toHaveBeenCalledWith('120363422153603317@g.us');
    });
});
...
describe('handleCall', () => {
    ...
    it('warns caller when anticall enabled', async () => {
        await handleCall(sock, [{ id: 'call-1', from: '263776695904@s.whatsapp.net' }]);
        expect(sock.sendMessage).toHaveBeenCalledWith(
            '263776695904@s.whatsapp.net',
    ...
    it('rejects call via rejectCall', async () => {
        await handleCall(sock, [{ id: 'call-1', from: '263776695904@s.whatsapp.net' }]);
        expect(sock.rejectCall).toHaveBeenCalledWith('call-1', '263776695904@s.whatsapp.net');
    });
    ...
    it('handles call without id gracefully', async () => {
        await expect(handleCall(sock, [{ from: '263776695904@s.whatsapp.net' }])).resolves.not.toThrow();
    });
});
